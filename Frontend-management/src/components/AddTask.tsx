...
if (task) {
  const {
    ...
    handleSubmit,
    formState: { errors },
    setValue, // Adding setValue to pre-fill form fields
  } = useForm<TaskFormData>();

  const [team, setTeam] = useState<string[]>(task.team || []);
  const [stage, setStage] = useState<string>(task.stage ? task.stage.toUpperCase() : LISTS[0]);
  const [priority, setPriority] = useState<string>(task.priority ? task.priority.toUpperCase() : PRIORITY[2]);
  ...
  }, [task, setValue]); // Re-run when the task object changes
